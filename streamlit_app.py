...
def format_phone(phone_number):
    numbers = re.sub(r'[^0-9]', '', str(phone_number or ''))
            ...
            if submit:
                if not customer_name:
                    ...
                    try:
                        ...
                        order_datetime = KST.localize(datetime.combine(order_date, order_time))
                        pickup_datetime = KST.localize(datetime.combine(pickup_date, pickup_time))
        ...
        try:
            ...
            with tab1:
                ...
                if st.session_state.get("selected_calendar_date"):
                    ...
                    df_orders['p_date_str'] = df_orders['pickup_datetime'].dt.strftime('%Y-%m-%d')
                    day_orders = df_orders[df_orders['p_date_str'] == sel_date_str].copy()
                    ...
                    if day_orders.empty:
                        ...
                        # 행 전체 클릭 지원을 위한 클릭 가능 레이아웃
                        cols_head = st.columns([1, 1.5, 2, 2, 1.5, 1.5, 1.5, 2])
                        cols_head[0].write("**ID**")
                        cols_head[1].write("**고객명**")
                        cols_head[2].write("**연락처**")
                        cols_head[3].write("**상품명**")
                        cols_head[4].write("**금액**")
                        cols_head[5].write("**픽업시각**")
                        cols_head[6].write("**결제**")
                        cols_head[7].write("**선택**")
                        st.markdown("<hr style='margin:4px 0 10px 0;'>", unsafe_allow_html=True)

                        for _, d_row in day_orders.iterrows():
                            o_id = int(d_row['id'])
                            c_bg = "#F3EEF9" if st.session_state.get("edit_order_id") == o_id else "#FFFFFF"
                            
                            c1, c2, c3, c4, c5, c6, c7, c8 = st.columns([1, 1.5, 2, 2, 1.5, 1.5, 1.5, 2])
                            c1.write(f"#{o_id}")
                            c2.write(f"{d_row['customer_name']}")
                            c3.write(f"{d_row['phone']}")
                            c4.write(f"{d_row['product_name']}")
                            c5.write(f"{d_row['amount']:,}원")
                            c6.write(f"{d_row['픽업시각']}")
                            c7.write(f"{d_row['payment_method']}")
                            if c8.button(f"✏️ 선택", key=f"btn_day_{o_id}", use_container_width=True):
                                st.session_state["edit_order_id"] = o_id

            with tab2:
                df_all = df_orders.copy()
                if not df_all.empty:
                    df_all['접수일시'] = df_all['created_at'].dt.strftime('%Y-%m-%d %H:%M')
                    df_all['픽업일시'] = df_all['pickup_datetime'].dt.strftime('%Y-%m-%d %H:%M')
                    
                    c_head = st.columns([1, 1.5, 2, 2, 1.5, 2, 2, 1.5, 2])
                    c_head[0].write("**ID**")
                    c_head[1].write("**고객명**")
                    c_head[2].write("**연락처**")
                    c_head[3].write("**상품명**")
                    ...
                    for _, a_row in df_all.iterrows():
                        oid = int(a_row['id'])
                        ac1, ac2, ac3, ac4, ac5, ac6, ac7, ac8, ac9 = st.columns([1, 1.5, 2, 2, 1.5, 2, 2, 1.5, 2])
                        ac1.write(f"#{oid}")
                        ac2.write(f"{a_row['customer_name']}")
                        ac3.write(f"{a_row['phone']}")
                        ac4.write(f"{a_row['product_name']}")
                        ac5.write(f"{a_row['amount']:,}원")
                        ac6.write(f"{a_row['접수일시']}")
                        ac7.write(f"{a_row['픽업일시']}")
                        ac8.write(f"{a_row['payment_method']}")
                        if ac9.button(f"✏️ 수정하기", key=f"btn_all_{oid}", use_container_width=True):
                            st.session_state["edit_order_id"] = oid

            # 하단 수정 폼 전용 부분 프래그먼트
            @st.fragment
            def render_edit_form_fragment():
                if st.session_state.get("edit_order_id") is not None:
                    ...
                    if not matched_rows.empty:
                        ...
                        orig_pdt = target_row['pickup_datetime'] if pd.notnull(target_row['pickup_datetime']) else get_kst_now()
                        ...
                        orig_cat = target_row['created_at'] if pd.notnull(target_row['created_at']) else get_kst_now()
                            ...
                            if update_submit:
                                if not edit_cname:
                                    ...
                                    try:
                                        ...
                                        new_order_dt = KST.localize(datetime.combine(edit_o_date, new_order_time))
                                        new_pickup_dt = KST.localize(datetime.combine(edit_p_date, new_pickup_time))
